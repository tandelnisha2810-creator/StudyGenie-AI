/**
 * Google-only Authentication Screen
 */
...
export default function AuthIndex() {
  ...
  const provider = useMemo(() => googleProvider ?? new GoogleAuthProvider(), []);
  ...
  const handleContinueWithGoogle = async () => {
    ...
    try {
      if (Platform.OS === "web") {
        ...
        // If user already completed redirect flow, handle it.
...
const styles = StyleSheet.create({
  ...
  googleButtonInner: { flexDirection: "row", alignItems: "center" },
  googleDot: {
    width: 18,
    height: 18,
    borderRadius: 9,
